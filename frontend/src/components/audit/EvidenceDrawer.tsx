...
export function EvidenceDrawer({ auditId, eventId, onClose }: { auditId: string; eventId: string; onClose: () => void }) {
  ...
  return <aside aria-label="FireEvent evidence drawer" className="absolute top-0 right-0 z-20 h-full w-[min(440px,92vw)] overflow-y-auto border-l border-border-strong bg-panel/98 text-text shadow-2xl">
    ...
    {state.data && <>
      <Section title="Summary"><div className="grid grid-cols-2 gap-2 text-xs"><div><span className="text-text-muted">Scope relation</span><div>{state.data.scopeRelation}</div></div><div><span className="text-text-muted">Sufficiency</span><div>{state.data.evidenceSufficiency.value}</div></div><div><span className="text-text-muted">Investigation priority</span><div>{state.data.investigationPriority}</div></div><div><span className="text-text-muted">Human workflow</span><div>{state.data.reviewState}</div></div><div><span className="text-text-muted">Chronology</span><div>{state.data.event.firstDetection.slice(0, 16)} → {state.data.event.lastDetection.slice(0, 16)}</div></div><div><span className="text-text-muted">Observations</span><div>{state.data.event.observationCount} · max FRP {state.data.event.maxFrp?.toFixed(2) ?? '—'} MW</div></div></div><p className="mt-2 text-[10px] text-text-muted">{state.data.evidenceSufficiency.reason}</p><p className="mt-1 text-[10px] text-text-muted">Priority and workflow are deterministic routing aids; neither establishes cause, responsibility, or exoneration.</p></Section>
