...
it('selects candidates from the full keyboard-accessible row surface', async () => {
  ...
  expect(screen.getByRole('button', { name: 'SHOW FIRMS' })).toBeTruthy()
  expect(screen.getByRole('button', { name: 'SHOW PEATLAND' })).toBeTruthy()
  expect(screen.getByRole('button', { name: 'SHOW SPREAD ENVELOPES' })).toBeTruthy()
  expect(screen.queryByText(/Lines are limited to 10 km/)).toBeNull()
  expect(screen.queryByText(/Dashed outline:/)).toBeNull()
  expect(screen.queryByRole('button', { name: /NIGHT/i })).toBeNull()
  expect(screen.queryByRole('button', { name: /SOIL MOISTURE/i })).toBeNull()
  expect(screen.queryByRole('button', { name: /GROUNDWATER/i })).toBeNull()
  expect(screen.queryByRole('button', { name: /PEATCLSM/i })).toBeNull()
...
it('enables the scoped FIRMS overlay without a focused FireEvent', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'SHOW FIRMS' }))
